...
`
	...
	// Verify SimpleChannels
	simple := spec.SimpleChannels()
	if len(simple) != 2 {
		t.Errorf("Expected 2 simple channels, got %d", len(simple))
	}
	for _, s := range simple {
		if s.Address == "user/signedup" {
			if len(s.Messages) != 1 || s.Messages[0] != "userSignedUp" {
				t.Errorf("Unexpected messages for user/signedup: %v", s.Messages)
			}
		} else if s.Address == "user/signedup/component" {
			if len(s.Messages) != 1 || s.Messages[0] != "inlineMsg" {
				t.Errorf("Unexpected messages for user/signedup/component: %v", s.Messages)
			}
		} else {
			t.Errorf("Unexpected channel address: %s", s.Address)
		}
	}
}

func TestSimpleChannels(t *testing.T) {
	yamlContent := `
asyncapi: '3.0.0'
channels:
  ch1:
...
`
	tmpDir := t.TempDir()
	tmpFile := filepath.Join(tmpDir, "simple_channels.yaml")
	if err := os.WriteFile(tmpFile, []byte(yamlContent), 0644); err != nil {
		t.Fatalf("Failed to write temp file: %v", err)
	}
